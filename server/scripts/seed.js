...
const answers = () => {
  for (var i = 0; i < 30; i++) {
    db.query(`INSERT INTO answers (questionsID, userID, text, votes, date) VALUES ("${faker.random.number(4) + 1}", "${faker.random.number(49) + 1}", "${faker.lorem.paragraph()}", "${faker.random.number(50)}", "${faker.date.past()}")`)
  }
}
